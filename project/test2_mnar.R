...
data <- read.csv(url("https://raw.githubusercontent.com/Bleak-bleak/ams572project/main/h2_mnar_imputed.csv"),header = TRUE)
data <- na.omit(data)
data$Y <- ifelse(data$num.of.doors =="two", 1, 0)

#load output
sink("results/mnar_output.txt")
...
jpeg("results/mnar_residual.jpg", width = 350, height = 350)
plot(fitted(stepACI.model), res, main = "mnar")
...
jpeg("results/mnar_cook.jpg", width = 350, height = 350)
plot(cooksD, main = "Cooks Distance for mnar")
