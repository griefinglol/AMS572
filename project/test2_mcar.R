...
data <- read.csv(url("https://raw.githubusercontent.com/Bleak-bleak/ams572project/main/h2_mcar_imputed.csv"),header = TRUE)
data <- na.omit(data)
data$Y <- ifelse(data$num.of.doors =="two", 1, 0)

#load output
sink("results/mcar_output.txt")
...
jpeg("results/mcar_residual.jpg", width = 350, height = 350)
plot(fitted(stepACI.model), res, main = "mcar")
...
jpeg("results/mcar_cook.jpg", width = 350, height = 350)
plot(cooksD, main = "Cooks Distance for mcar")
